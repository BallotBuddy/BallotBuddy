...
opensecrets.checkStashReturn = function (cid) {
  return db.queryFunding(cid).then(function (data) {
        if (data.length < 1) {
            return opensecrets.candSectorFunding(cid).then(function () {
...
// Takes a candidates opensecrets id
...
industryCodes = [
    { code: 'A01', industry: 'Crop Production & Basic Processing', sector: 'Agribusiness' },
